...
Because $\sigma$ represents a characteristic atomic length scale in the Lennard-Jones potential,

$$
\sigma^3
$$

provides a volume-like quantity.
...
For C-terminal residues, the terminal
...
so torsional barriers closer to the alpha carbon contribute more strongly than those located farther away in the residue topology.
...
where `XXX` identifies the amino acid or modified amino acid.
...
|---|---|---|
| **F1 Net Charge** | $Q_{\mathrm{raw}}-Q_{\mathrm{bias}}$ | Atomic partial charges |
| **F2 Charge Variance** | $\mathrm{Var}(q_i)$ | Atomic partial charges |
| **F3 vdW Volume Proxy** | $\sum_i\sigma_i^3$ | LJ $\sigma$ |
| **F4 Dispersion Capacity** | $\sum_{\mathrm{heavy}}\epsilon_i$ | LJ $\epsilon$ |
| **F5 Donor Strength** | $\sum q_{H,\mathrm{polar}}$ | Atomic charges + bonding topology |
| **F6 Acceptor Strength** | $\sum_{i \in \{N,O\}} \lvert q_i \rvert$ | Atomic charges + atom identity |
| **F7 Torsional Rigidity** | $\sum_k\Delta E_k/d_k$ | Amber dihedral parameters + topology |
...
which returns the full feature dictionary.
